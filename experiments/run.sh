...
case "${1:-help}" in
    ...
    *)
        ...
        echo "  --docgen-only        Re-run doc-gen4 + HTML (loads classification from cache)"
        echo "  --html-only          Regenerate HTML only (loads classification from cache)"
        ...
        echo "  $0 run --html-only --projects mathlib4   # Regenerate HTML (uses cache)"
